...
zx_status_t IommuImpl::ValidateIommuDesc(const fbl::unique_ptr<const uint8_t[]>& desc_bytes,
    ...
    scope_bytes *= sizeof(zx_iommu_desc_intel_scope_t);
    ...
    while (cursor_bytes + sizeof(zx_iommu_desc_intel_reserved_memory_t) < desc_len) {
        ...
        next_entry += sizeof(zx_iommu_desc_intel_reserved_memory_t);
...
zx_status_t IommuImpl::Map(uint64_t bus_txn_id, paddr_t paddr, size_t size, uint32_t perms,
                           dev_vaddr_t* vaddr) {
    DEBUG_ASSERT(vaddr);
...
zx_status_t IommuImpl::EnableBiosReservedMappingsLocked() {
    ...
    while (cursor_bytes + sizeof(zx_iommu_desc_intel_reserved_memory_t) < desc->reserved_memory_bytes) {
        ...
        for (size_t i = 0; i < num_scopes; ++i) {
            ...
            DeviceContext* dev;
            zx_status_t status = GetOrCreateDeviceContextLocked(scopes[i].start_bus,
    ...
    // table management
    __asm__ volatile("wbinvd" : : : "memory");
    zx_status_t status = InvalidateContextCacheGlobalLocked();
    if (status != ZX_OK) {
        return status;
    }
...
zx_status_t IommuImpl::InvalidateContextCacheGlobalLocked() {
...
zx_status_t IommuImpl::InvalidateIotlbGlobalLocked() {
    DEBUG_ASSERT(lock_.IsHeld());
...
zx_status_t IommuImpl::GetOrCreateContextTableLocked(uint8_t bus, uint8_t dev_func,
    ...
    zx_status_t status = ContextTableState::Create(bus, supports_extended_context_,
                                                   dev_func >= 0x80 /* upper */,
                                                   this,
                                                   target_entry,
                                                   &table);
...
zx_status_t IommuImpl::GetOrCreateDeviceContextLocked(uint8_t bus, uint8_t dev_func,
                                                      DeviceContext** context) {

    DEBUG_ASSERT(lock_.IsHeld());

    ContextTableState* ctx_table_state;
    zx_status_t status = GetOrCreateContextTableLocked(bus, dev_func, &ctx_table_state);
    if (status != ZX_OK) {
        return status;
    }
